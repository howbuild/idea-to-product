#!/usr/bin/env python3
"""Review whether a feature candidate drifts from Core Intent and MVP scope."""
    ...
    path.write_text(
        "\n".join(
            [
                ...
                f"- MVP inclusion status: {result['mvp_inclusion_status']}",
                "",
                "## 장점",
                result["benefit"],
                "",
...
def main() -> int:
    ...
    candidate = (
        payload.get("candidate_feature")
        or payload.get("user_prompt")
        or payload.get("prompt")
        or "Unspecified feature candidate"
    )
    decision, reason = decide(candidate)

    if decision == "Green":
        recommendation = "MVP에 포함 가능"
        required_confirmation = "권장 사항을 사용자에게 보여주고 DECISION_LOG에 기록합니다."
    elif decision == "Yellow":
        recommendation = "축소 포함 또는 v2 후보로 검토"
        required_confirmation = "사용자가 선택하기 전까지 MVP에 확정 반영하지 않습니다."
    else:
        recommendation = "v2 이동 또는 보류 권장"
        required_confirmation = "MVP 반영 전 사용자 명시 확인이 필요합니다."

    result = {
        "candidate_feature": candidate,
        "decision": decision,
        "decision_reason": reason,
        "mvp_inclusion_status": "pending_user_confirmation" if decision in {"Yellow", "Red"} else "reviewed",
